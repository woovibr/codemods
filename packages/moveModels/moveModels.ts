...
(async () => {
  try {
    ...
    for (const model of models) {
      ...
      const internalImports = await runCodemodFile(getInternalImports, modelWithExtension);

      if (internalImports.length > 0) {
        // eslint-disable-next-line
        const imports = internalImports.map((i) => getRelativePath(modelpath, i.source.value));
